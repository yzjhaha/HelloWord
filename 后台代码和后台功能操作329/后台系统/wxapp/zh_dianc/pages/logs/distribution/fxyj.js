...
Page({
  ...
  formSubmit: function (e) {
    ...
    var rate = this.data.fxset.tx_rate, txmoney = Number(this.data.fxset.tx_money), je = e.detail.value.je, name = e.detail.value.name, zh = e.detail.value.zh, cb = e.detail.value.checkbox.length, zhlx = e.detail.value.radiogroup;
    console.log(user_id,useryj,rate,txmoney,je,name,zh,cb,zhlx)
    if(zhlx=='zfbtx'){
      var actype=1
    }
    if (zhlx == 'wxtx') {
    ...
    if (useryj <txmoney) {
      ...
      app.util.request({
        ...
        'cachetime': '0',
        data: { user_id: user_id, type: actype, user_name: name, account: zh, tx_cost: je, sj_cost:sjje },
