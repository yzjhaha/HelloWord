...
Page({
  ...
  select:function(e){
    ...
    if (that.data.state == '2') {
      wx: wx.redirectTo({
        url: '../order/order?&tableid=' + that.data.options.tableid,
        success: function (res) { },
        fail: function (res) { },
  ...
  use:function(e){
    ...
    for(var i = 0;i<coupons.length;i++){
      if(id==coupons[i].id){
        ...
        if (that.data.state == '2') {
          wx: wx.redirectTo({
            url: '../order/order?coupons_id=' + coupon.coupons_id + '&preferential=' + coupon.preferential + '&tableid=' + that.data.options.tableid,
            success: function (res) { },
            fail: function (res) { },
            complete: function (res) { },
          })
  ...
  user: function (e) {
    ...
    for (var i = 0; i < Voucher.length; i++) {
      if (id == Voucher[i].id) {
        ...
        if (that.data.state == '2') {
          wx: wx.redirectTo({
            url: '../order/order?vouchers_id=' + coupon.vouchers_id + '&preferential=' + coupon.preferential + '&tableid=' + that.data.options.tableid,
            success: function (res) { },
            fail: function (res) { },
            complete: function (res) { },
          })
