...
if($_GPC['keywords']){
    ...
    $sql="select * from ".tablename("wpdc_ydorder")."where uniacid=:uniacid and store_id=:store_id and state!=0";
        if($type=='all'){
            if(checksubmit('submit2')){
                $sql.=" and time2 >= :time2 and time2 <= :time22 and state!=0";
        $data=array(':time2'=>$start,':time22'=>$end,':uniacid'=>$_W['uniacid'],':store_id'=>$storeid);
        $total=pdo_fetchcolumn("SELECT count(*) FROM ".tablename('wpdc_ydorder')." WHERE time2 >= :time2 and time2 <= :time22 and uniacid=:uniacid and state!=0 and store_id=:store_id ORDER BY time2 DESC",$data);
        }elseif($_GPC['keywords']){
            $sql.="  and (link_name LIKE :name|| order_num LIKE :order_num) and state!=0  ";
            $data=array(':name' => $where,':order_num' => $where,':uniacid'=>$_W['uniacid'],':store_id'=>$storeid);
            $total=pdo_fetchcolumn("SELECT count(*) FROM ".tablename('wpdc_ydorder')." WHERE (link_name LIKE :name || order_num LIKE :order_num) and state!=0  and uniacid=:uniacid   and store_id=:store_id ORDER BY time2 DESC",$data);
        }else{
            $sql=$sql;
            $data=array(':uniacid'=>$_W['uniacid'],':store_id'=>$storeid);
            $total=pdo_fetchcolumn("select count(*) from " .tablename('wpdc_ydorder')."where uniacid=:uniacid and state!=0  and store_id=:store_id",$data);
            //$total=pdo_fetchcolumn($sql,$data);
        }
        }elseif($type=='wait'){
                $sql.=" and state=1";
                $data=array(':uniacid'=>$_W['uniacid'],':store_id'=>$storeid);
                $total=pdo_fetchcolumn("select count(*) from " .tablename('wpdc_ydorder')."where uniacid=:uniacid  and state=1  and store_id=:store_id",$data);
        }elseif($type=='complete'){
            $sql.=" and state=2";
            $data=array(':uniacid'=>$_W['uniacid'],':store_id'=>$storeid);
            $total=pdo_fetchcolumn("select count(*) from " .tablename('wpdc_ydorder')."where  uniacid=:uniacid and state=2  and store_id=:store_id",$data);
        }elseif($type=='reject'){
            $sql.=" and state=3";
            $data=array(':uniacid'=>$_W['uniacid'],':store_id'=>$storeid);
            $total=pdo_fetchcolumn("select count(*) from " .tablename('wpdc_ydorder')."where uniacid=:uniacid  and state=3 and store_id=:store_id",$data);
        }elseif($type=='cancel'){
            $sql.=" and state=4";
            $data=array(':uniacid'=>$_W['uniacid'],':store_id'=>$storeid);
            $total=pdo_fetchcolumn("select count(*) from " .tablename('wpdc_ydorder')."where uniacid=:uniacid  and state=4 and store_id=:store_id",$data);
        }
        $select_sql =$sql." order by time2  desc LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
