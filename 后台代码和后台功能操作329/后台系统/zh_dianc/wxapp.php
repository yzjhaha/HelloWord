...
public function doPageAppDnOrder(){
  global $_W, $_GPC;
    ...
    $res=pdo_fetchall($sql,array(':seller_id'=>$_GPC['store_id'],':uniacid'=>$_W['uniacid']));
    echo json_encode($res);
}
...
public function doPageAppYdOrder(){
	global $_W, $_GPC;
	$res=pdo_getall('wpdc_ydorder',array('store_id'=>$_GPC['store_id'],'state !='=>0),array(),'','id DESC');
	echo json_encode($res);
}
...
public function doPageAppDmOrder(){
	global $_W, $_GPC;
	$sql="SELECT a.*,b.md_name,c.name FROM ".tablename('wpdc_dmorder'). " a"  . " left join " . tablename("wpdc_store") . " b on a.store_id=b.id " . " left join " . tablename("wpdc_user") . " c on c.id=a.user_id WHERE a.uniacid=:uniacid and a.store_id=:store_id and a.state=2 ORDER BY a.time2 DESC";
	$res=pdo_fetchall($sql,array(':store_id'=>$_GPC['store_id'],':uniacid'=>$_W['uniacid']));
    echo json_encode($res);
}
//菜品分类
